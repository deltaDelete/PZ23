...
public class RequestStatusViewModel: ViewModelBase {
    private List<RequestStatus> _itemsFull = null!;
    ...
    [Reactive] public BindingList<RequestStatus> Items { get; set; } = new();
    ...
    [Reactive] public List<RequestStatus> Filtered { get; set; } = new List<RequestStatus>();

    [Reactive] public bool IsLoading { get; set; } = true;

    [Reactive] public RequestStatus? SelectedRow { get; set; } = null!;

    #endregion

    public ReactiveCommand<RequestStatus, Unit> EditItemCommand { get; }
    public ReactiveCommand<RequestStatus, Unit> RemoveItemCommand { get; }
    ...
    public RequestStatusViewModel() {
        ...
        var canEdit = this.WhenAnyValue(
            x => x.SelectedRow, 
            selector: request_status => request_status is not null 
        ...
        EditItemCommand = ReactiveCommand.Create<RequestStatus>(EditItem, canEdit);
        RemoveItemCommand = ReactiveCommand.Create<RequestStatus>(RemoveItem, canEdit);
    ...
    private void OnSearchChanged((string query, int column, bool isDescending) tuple) {
        ...
        var filtered = tuple.query == ""
            ...
            : tuple.column switch {
                1 => _itemsFull
                    .Where(it => it.RequestStatusId.ToString().Contains(tuple.query)),
                2 => _itemsFull
                    .Where(it => it.RequestStatusName.ToLower().Contains(tuple.query.ToLower())),
                _ => _itemsFull
                    .Where(
                        it => it.RequestStatusName.ToLower().Contains(tuple.query.ToLower()) ||
                                   it.RequestStatusId.ToString().Contains(tuple.query.ToLower())
                        )
            };

        Filtered = tuple.column switch {
            2 => tuple.isDescending
                ? filtered.OrderByDescending(it => it.RequestStatusName).ToList()
                : filtered.OrderBy(it => it.RequestStatusName).ToList(),
            _ => tuple.isDescending
                ? filtered.OrderByDescending(it => it.RequestStatusId).ToList()
                : filtered.OrderBy(it => it.RequestStatusId).ToList()
        };
    }
    ...
    private async void GetDataFromDb() {
        await Task.Run(async () => {
            ...
            var list = await db.GetAsync<RequestStatus>().ToListAsync();
            _itemsFull = list;
            Filtered = _itemsFull;
            IsLoading = false;
    ...
    private async void RemoveItem(RequestStatus? arg) {
    ...
    private void RemoveLocal(RequestStatus arg) {
        Items.Remove(arg);
        _itemsFull.Remove(arg);
        Filtered.Remove(arg);
    }

    private async void EditItem(RequestStatus? arg) {
        if (arg is null) return;
    ...
    private void ReplaceItem(RequestStatus prevItem, RequestStatus newItem) {
